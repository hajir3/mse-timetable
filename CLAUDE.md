...
```
pnpm install
cp apps/web/.env.example apps/web/.env.local   # fill in real Clerk keys
pnpm dev      # regenerates the dataset (predev), then starts apps/web
pnpm build    # regenerates the dataset (prebuild), then builds apps/web
```

`apps/web` needs a real Clerk application (dashboard.clerk.com) — Google
OAuth enabled, publishable + secret key in `.env.local` — to actually sign
in; without it, `next dev`/`next build` still compile and lint clean, but
any page behind `middleware.ts`'s `auth.protect()` won't resolve.

`apps/web/AGENTS.md` (and its `apps/web/CLAUDE.md`, which just references
it) are auto-generated by `next dev`/`next build` itself (Next.js 16's
"agent rules" feature) to flag that this Next.js version has breaking
changes an agent's training data may predate — read it before touching
Next.js/Clerk APIs. It regenerates on every run and is meant to be
committed; don't delete it.
